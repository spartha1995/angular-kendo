...
let SplitterService = class SplitterService {
    ...
    dragState(splitbarIndex) {
        const previous = this.pane(splitbarIndex);
        const next = this.pane(splitbarIndex + 1);
        // determine which pane to resize
        let index, initial;
        if (next.collapsible || next.fixedSize) {
            index = splitbarIndex + 1;
            initial = next.computedSize;
        }
        else {
            index = splitbarIndex;
            initial = previous.computedSize;
        }
        // calculate resizing constraints
        const max = previous.computedSize + next.computedSize;
        const px = s => this.toPixels(s);
        return {
            initial,
            index,
            min: px(previous.min) || max - px(next.max) || 0,
            max: px(previous.max) || max - px(next.min) || max
        };
    }
    setSize(state, delta) {
...
SplitterService = tslib_1.__decorate([
    Injectable()
], SplitterService);
